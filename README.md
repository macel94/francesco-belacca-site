...
- Reliability page with current native-production architecture, GitOps delivery,
  ...
  practice, backup destination evidence, gated production automation, and
  recovery/failover gaps. Planned controls are labeled as planned; no secrets or
...
status repository and falls back to checked-in `unknown` with “awaiting fresh evidence” copy when
