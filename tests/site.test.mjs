...
test('site discovery assets are linked and shipped', async () => {
  ...
  for (const document of ['index.html', 'privacy.html', 'reliability.html', 'status.html']) {
    ...
    assert.match(documentHtml, /<link rel="icon" href="\/favicon-tab\.png" type="image\/png" sizes="32x32" \/>/);
    assert.doesNotMatch(documentHtml, /href="\/favicon\.ico"/);
    assert.doesNotMatch(documentHtml, /href="\/icon-32\.png"/);
    assert.doesNotMatch(documentHtml, /href="\/icon-16\.png"/);
  ...
  for (const asset of ['index.html', 'reliability.html', 'reliability.js', 'privacy.html', 'styles.css', 'app.js', 'count.js', 'favicon.svg', 'favicon-tab.png', 'favicon.ico', 'icon-16.png', 'icon-32.png', 'apple-touch-icon.png', 'icon-192.png', 'icon-512.png', 'site.webmanifest', 'robots.txt', 'llms.txt', 'sitemap.xml']) assert.match(dockerfile, new RegExp(asset.replace('.', '\\.'), 'u'));
...
test('the supplied canonical mark is used across public branding surfaces', async () => {
  ...
  const tabPng = await readBytes('favicon-tab.png');
  assert.match(svg, /viewBox="0 0 512 512"/);
  assert.deepEqual(tabPng.subarray(0, 8), Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]));
  assert.equal(tabPng.readUInt32BE(16), 32);
  assert.equal(tabPng.readUInt32BE(20), 32);
  // Generated by rasterizing favicon.svg at 32x32 with Chromium. This pins
  // the tab derivative to the canonical in-page mark instead of a separate design.
  assert.equal(createHash('sha256').update(tabPng).digest('hex'), '7d5fc66d522265c03a39770c90bd1969c4e771e612063479b820acd4bd01dd7a');
  ...
  for (const document of ['index.html', 'privacy.html', 'reliability.html', 'status.html']) {
    const html = await read(document);
    assert.match(html, /<link rel="icon" href="\/favicon-tab\.png" type="image\/png" sizes="32x32" \/>/);
